...
export const buildCommentBody = ({
  ...
  const runtime = `Node.js v${versions.node} and npm v${versions.npm}`;

  return `
<details>
<summary>Diff between <code>${from.name}</code> v${from.version} and v${to.version}</summary>
...
- Size: ${bytesToSize(from.size)} → **${bytesToSize(to.size)}** (${sizeDiff})
- Files: ${from.fileCount} → **${to.fileCount}** (${filesDiff})
- Command: \`${cmdLine}\`
