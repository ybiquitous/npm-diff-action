...
export const getPackageInfo = async (name, version) => {
  ...
  const array = JSON.parse(ret);
  if (!Array.isArray(array) || !array[0]) {
    throw new Error(`expect "npm view" to return an array, but actual: ${ret}`);
  }
  const info = array[0];
...
const maxCharsFromError = (error) => {
  ...
  const limit = matched?.groups?.["limit"];
...
const run = async () => {
  ...
  let diff;
  try {
    diff = await core.group("Run npm diff", () => runCommand(cmd, cmdArgs));
  } catch (error) {
    if (error instanceof Error && /\bE404\b/u.test(error.message)) {
      core.info(`The package "${updateInfo.name}" was not found on npm. Skip npm diff.`);
      return;
    }
    throw error;
  }
